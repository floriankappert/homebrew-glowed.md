# homebrew-tap

Homebrew tap for Florian Kappert's builds.

```bash
brew tap floriankappert/tap
```

## Formulae

### glowed

A distribution of [glowed](https://github.com/khw1031/glowed), a Ghostty-oriented terminal
TUI Markdown browser/editor by [@khw1031](https://github.com/khw1031).

```bash
brew install floriankappert/tap/glowed
```

- Source: https://github.com/floriankappert/glowed
- Binary: `glowed` (drop-in replacement for the upstream build)
- Versioning: `vX.Y.Z-floriankappert.N`, where `vX.Y.Z` is the upstream base

Upstream's contribution model asks downstream builds to be published as their own tap
rather than as pull requests; see the upstream
[CONTRIBUTING.md](https://github.com/khw1031/glowed/blob/main/CONTRIBUTING.md).
